...
#define s0 4  //color sensor pins
#define s1 5
#define s2L A4 //left sensor
#define s3L A5
#define outL A3
#define s2R A1 //right sensor
#define s3R A2
#define outR A0
int speed = 150;
const int DISTANCE_THRESHOLD = 15; // centimeters
short int duration_us, distance_cm;
//Left color sensor values
int redL = 0;
int greenL = 0;
int blueL = 0;
//Right color sensor values
int redR = 0;
int greenR = 0;
int blueR = 0;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void rgblight(String color)
{
  if(color == "red"){pin = 33;}
  else if(color == "green"){pin = 25;}
  else{pin = 24;}
}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//-------------------------------------------------------WI-FI------------------------------------------------------------//
...
String getIPaddress()
{
  ...
  int startIP = response.indexOf("+CIFSR:APIP");
...
void WiFi_initialize()
{
  ...
  sendData("AT+CWJAP=\"STUDENTS\",\"auston@0426JS\"\r\n",5000,DEBUG); //connect wifi  
...
void LeaveHome()
{
  if(color == "red" && lastColor == 'r'){while(sensor_left == 0 || sensor_right == 0){ColorDetect();Red();turnRight();}forward();delay(300);}
  else if(color == "red" && lastColor == 'g'){while(sensor_left == 0 || sensor_right == 0){ColorDetect();Red();turnRight();}forward();delay(300);}
  else if(color == "red" && lastColor == 'b'){while(sensor_left == 0 || sensor_right == 0){ColorDetect();Red();turnLeft();}forward();delay(300);}
  else if(color == "green" && lastColor == 'r'){while(sensor_left == 0 || sensor_right == 0){ColorDetect();Green();turnLeft();}forward();delay(300);}
  else if(color == "green" && lastColor == 'g'){while(sensor_left == 0 || sensor_right == 0){ColorDetect();Green();turnRight();}forward();delay(300);}
  else if(color == "green" && lastColor == 'b'){while(sensor_left == 0 || sensor_right == 0){ColorDetect();Green();forward();}forward();delay(300);}
  else if(color == "blue" && lastColor == 'r'){while(sensor_left == 0 || sensor_right == 0){ColorDetect();Blue();turnRight();}forward();delay(300);}
  else if(color == "blue" && lastColor == 'g'){while(sensor_left == 0 || sensor_right == 0){ColorDetect();Blue();forward();}forward();delay(300);}
  else if(color == "blue" && lastColor == 'b'){while(sensor_left == 0 || sensor_right == 0){ColorDetect();Blue();turnLeft();}forward();delay(300);}
}
...
void FoundYellow()
{
  ...
  {
    stop();display_whenReach();play_sound(color);
    //Serial.println("Reached yellow!STOPPED");
    while(color != "yellow"){receiveInfo();}
  }
}
...
void FoundYellowHome()
{
  ...
  {
    forward();
    if(colorToReturn == "red"){delay(200);}else{delay(400);}
    stop();display_whenReach();digitalWrite(pin,LOW);
...
void Red()
{ //Sensor Left
  if (redL <= 20 && greenL >= 15 && blueL >= 8 && redL < blueL && redL < greenL)
  ...
  //Sensor Right
  if (redR <= 15 && greenR >= 10 && blueR >= 10 && redR < blueR && redR < greenR)
...
void Green()
{  //Sensor Left
  if (greenL <= 20 && redL >= 8 && blueL >= 5 && greenL < redL)
  ...
  //Sensor Right
  if (greenR <= 20 && redR >= 9 && blueR >= 9 && greenR < redR)
...
{
  //Sensor Left
  if (blueL <= 20 && redL >= 15 && greenL >= 10 && blueL < redL && blueL < greenL)
  ...
  //Sensor Right
  if (blueR <= 25 && redR >= 20 && greenR >= 20 && blueR < redR && blueR < greenR)
...
void display_rollingHome()
{
  static const unsigned char image_FaceCharging_29x14_bits[] U8X8_PROGMEM = {0x40,0x00,0x00,0x02,0x60,0x00,0x00,0x03,0x30,0x00,0x80,0x01,0x18,0x00,0xc0,0x00,0xfc,0x00,0xe0,0x07,0xfc,0x00,0xe0,0x07,0x60,0x00,0x00,0x03,0x30,0x00,0x80,0x01,0x18,0x00,0xc0,0x00,0x08,0x00,0x40,0x00,0x00,0x00,0x00,0x00,0x00,0x04,0x04,0x00,0x00,0x08,0x02,0x00,0x00,0xf0,0x01,0x00};  
   u8g2.firstPage();
   do {
    u8g2.setFont(u8g2_font_seraphimb1_tr);//u8g2_font_tenthinnerguys_t_all);
    u8g2.drawStr(5, 13, "Rollin' Back Home");
