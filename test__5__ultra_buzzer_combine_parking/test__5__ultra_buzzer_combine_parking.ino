...
void setup()
{
  ...
  Serial1.begin(115200);//for communication with esp
  ...
  digitalWrite(ECHO_PIN, LOW);  //set low 
  ...
  if(distance_cm > DISTANCE_THRESHOLD)
  {
    if(color == "red" || color == "green" || color == "blue")
   {
      ...
      while(color != "yellow")
      {
        ...
        if(distance_cm > DISTANCE_THRESHOLD)
        {
          display_status(color);ColorDetect();executeColor(color);Direction();FoundYellow();
        }
        else{stop();Alarm();}
      }
    }
...
void FoundYellowHome()
{
  ...
  if(yellow_left == 1 && yellow_right == 1 && sensor_left == 0 && sensor_right == 0)
  {
    ...
    {
      forward();delay(500);
      while(sensor_left == 0 || sensor_right == 0){ColorDetect();Red();turnLeft();delay(50);}backward();delay(250);
    }
    else if(colorToReturn == "blue")
    {
      forward();delay(500);
      while(sensor_left == 0 || sensor_right == 0){ColorDetect();Red();turnRight();delay(50);}backward();delay(250);
    }
